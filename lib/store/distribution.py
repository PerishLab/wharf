...
def status(name, decided, result):
    decision, presence = decided
    if result == "success":
        return DONE[name]
...
def observed(context, needs):
    ...
    presence = json.loads(decided.get("presence") or "{}")
...
def final(held):
    return held in DONE.values() or held in KNOWN
