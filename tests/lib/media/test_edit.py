...
class Edit(unittest.TestCase):
    def setUp(self):
        ...
        self.first = edit.stage(self.bucket, edit.Change(release("v0.38.0-beta.8"), edit.directory(root), None, NOW))

    def test_full_publish_records_media_and_modes(self):
        own = lineage.standing(self.bucket, "beta", "v0.38.0-beta.8")
    ...
    def test_patch_continues_lineage_and_copies_unchanged_objects(self):
        base = lineage.base(self.bucket, ("beta", "v0.38.0-beta.8"))
        change = tree({"help/new.txt": (b"new", 0o644)})
        content = edit.patched(base, [("help/new.txt", change / "help/new.txt")], ["assets/hook"])
        result = edit.stage(self.bucket, edit.Change(release("v0.38.0-beta.8"), content, base, NOW))
        ...
        now = lineage.standing(self.bucket, "beta", "v0.38.0-beta.8")
    ...
    def test_new_marker_inherits_the_nearest_on_its_line(self):
        base = lineage.base(self.bucket, ("beta", "v0.38.0-beta.9"))
        self.assertEqual(base["version"], "v0.38.0-beta.8")
        result = edit.stage(self.bucket, edit.Change(release("v0.38.0-beta.9"), edit.patched(base, [], []), base, NOW))
        self.assertIsNone(result["previous"])
        with self.assertRaisesRegex(Refusal, "--full or --from"):
            lineage.base(self.bucket, ("beta", "v0.39.0-beta.1"))

    def test_from_names_a_route_or_a_digest(self):
        by_route = lineage.base(self.bucket, ("beta", "v0.39.0-beta.1"), "beta/v0.38.0-beta.8")
        by_digest = lineage.base(self.bucket, ("beta", "v0.39.0-beta.1"), self.first["generation"])
    ...
    def test_concurrent_pointer_moves_are_refused(self):
        ...
        with self.assertRaisesRegex(Refusal, "pull again"):
            edit.stage(self.bucket, edit.Change(release("v0.38.0-beta.8"), content, base, NOW))
